  ...
  async spin(bet: number): Promise<SpinResult> {
    ...
    for (let r = 0; r < count; r += 1) {
      ...
      for (let row = 0; row < rows; row += 1) {
        col.push(this.pickSymbolId(r, row));
      }
      stops.push(col);
    }
    ...
    console.log("result :", { stops, wins, totalWin, scatterCount, awardedFreeSpins });

    return { stops, wins, totalWin, scatterCount, awardedFreeSpins };
  }

  private pickSymbolId(reelIndex: number, rowIndex: number): SymbolId {
